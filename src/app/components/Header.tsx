  ...
  return (
    <header id="home" className="fixed top-0 left-0 w-full z-50 bg-white/80 backdrop-blur shadow transition-all">
      <div className="container mx-auto flex items-center justify-between py-4 px-4">
        ...
        <button
          className={`md:hidden text-2xl text-tiffany focus:outline-none z-50 ml-auto transition-transform duration-300 ${menuOpen ? 'translate-x-[-16rem]' : ''}`}
      ...
      {menuOpen && (
        <>
          ...
          >
            <nav className="flex flex-col items-center justify-center h-full gap-8 text-lg font-semibold pt-24">
              {navLinks.map((link, idx) => (
                <a
                  key={link.href}
                  href={link.href}
                  className={`hover:text-tiffany transition-colors${idx === 0 ? ' mt-8' : ''}`}
                  onClick={e => {
                    e.preventDefault();
